...
#endif /* WITH_xMIPv6 */
{
    ...
    if ( addr.isGlobal() )  //only tag a global scope address as HoA or CoA, depending on the status of the H-Flag
    {
        if (hFlag == true)
        {
            ...
            fprintf(stdout, "HoA assigned!");
        }
        else
        {
            fprintf(stdout, "CoA assigned!");
...
void IPv6InterfaceData::choosePreferredAddress()
{
    ...
    fprintf(stdout, "\nchooseprefferedaddress\n");
    for (int i = 0; i < (int)addresses.size(); ++i)
    {
        //fprintf(stdout, "%d\t", addresses[i]);
    }
    fprintf(stdout, "\n");
...
{
    fprintf(stdout, "getglobaladdress \n");
    for (AddressDataVector::const_iterator it=addresses.begin(); it!=addresses.end(); it++)
        fprintf(stdout, "addrtype:%d addr:%d\t", it->addrType, it->address);

    for (AddressDataVector::const_iterator it=addresses.begin(); it!=addresses.end(); it++)
        if (it->address.isGlobal() && it->addrType == type )
        {
            // FIXME and valid, 25.9.07 - CB
