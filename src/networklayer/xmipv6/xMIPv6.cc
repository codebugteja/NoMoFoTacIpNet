...
void xMIPv6::handleMessage(cMessage *msg)
{
    ...
    else if (dynamic_cast<MobilityHeader*>(msg))
    {
        ...
        fprintf(stdout, "Mobility(binding related) msg received!!\n");
        ...
        fprintf(stdout, "yo11100 NODE: %d IID: %d GA: %d\n", ctrlInfo->getDestAddr(), ctrlInfo->getInterfaceId(), ie->ipv6Data()->getGlobalAddress());
...
{
    IPv6Address buDest = ctrlInfo->getDestAddr();
    InterfaceEntry *ie = ift->getInterfaceById(ctrlInfo->getInterfaceId());
    IPv6Address bu, bu1 = ie->ipv6Data()->getGlobalAddress();
...
void xMIPv6::initiateMIPv6Protocol(InterfaceEntry *ie, const IPv6Address& CoA)
{
    ...
    IPv6Address haDest = ie->ipv6Data()->getHomeAgentAddress(); // HA address for use in the BU for Home Registration
                    ...
                    //BindingUpdateList::BindingUpdateListEntry* entry = bul->lookup(haDest);
            removeCoAEntries(); // TODO would be better if this is done somewhere else or in a comletely different way
                    interfaceCoAList[ie->getInterfaceId()] = CoA;

                    tunneling->createTunnel(IPv6Tunneling::NORMAL, CoA, haDest); // update 10.06.08 - CB
                    tunneling->createTunnel(IPv6Tunneling::NORMAL, haDest, CoA); // update 10.06.08 - CB
...
void xMIPv6::sendPeriodicBU(cMessage *msg)
{
    ...
    if (!buIfEntry->homeRegistration) // this BU goes to a CN
    {
        ...
        fprintf(stdout, "\nnkj1 %d %d\n", buIfEntry->dest, ie->ipv6Data()->getGlobalAddress());
    ...
    {
        fprintf(stdout, "\nnkqej1 %d\n", ie->ipv6Data()->getGlobalAddress(IPv6InterfaceData::CoA));
    ...
    */
    ...
    if (validBUMessage)
    {
        ...
        else
        {
            ...
            fprintf(stdout, "BU reg!\n");
            if (homeRegistration)
            {
                fprintf(stdout, "BU reg HA!\n");
                /* 10.3.1
            ...
            else // CN, update 18.9.07 - CB
            {
                ...
                fprintf(stdout, "psuedo tunnel created!!\n");
        ...
        if (ba->getStatus() < 128)
        {
            ...
            else
            {
                ...
                if (entry->BAck == false && entry->destAddress == ie->ipv6Data()->getHomeAgentAddress()) // BA from HA
                {
                    ...
                    fprintf(stdout, "BLAAA33!\n");
                    for (itCNList = cnList.begin(); itCNList != cnList.end(); itCNList++) // run an iterator through the CN map
                    {

                        fprintf(stdout, "BLAAA22!\n");
                        IPv6Address& cnDest = *(itCNList);
...
bool xMIPv6::checkForBUtoCN(BindingUpdateList::BindingUpdateListEntry& bulEntry, InterfaceEntry *ie)
{
    ...
    {
        // for a registration BU we need both valid care-of and home keygen token
...
xMIPv6::TimerIfEntry* xMIPv6::getTimerIfEntry(Key& key, int timerType)
{
    ...
    if (pos != transmitIfList.end())
    {
        ...
        if (dynamic_cast<BUTransmitIfEntry*>(pos->second))
        {
